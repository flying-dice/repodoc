...
import type { MemFileSystemAdapter } from '../src/adapters/memFileSystem';
import { parseFrontmatter } from '../src/frontmatter';
import type { RepoDocStore } from '../src/store';
...
function seedBoard(
  columnIds: string[],
  cards: Array<{ slug: string; col: string }>,
): {
...
describe('store.moveCard — bookkeeping', () => {
  ...
  test('files stay contiguously numbered from 01 across an arbitrary move sequence', () => {
    ...
    checkInvariant();
    for (let i = 0; i < 20; i++) {
      const slug = required(slugs[Math.floor(rand() * slugs.length)], 'random slug');
      const toCol = required(columns[Math.floor(rand() * columns.length)], 'random column');
