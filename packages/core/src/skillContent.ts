...
RepoDoc ships a command line that drives the very same store as the VS Code
extension, so slugs, renumbering, gate checks, and the comment/gate line
formats always come out right. Run it from the repo root with Bun:

\`\`\`sh
bunx github:flying-dice/repodoc board list
bunx github:flying-dice/repodoc board show <board>
bunx github:flying-dice/repodoc card show <board> <card> [--json]
bunx github:flying-dice/repodoc card create <board> "<title>" [--column <id>] [--priority high|med|low] [--labels a,b]
...
The extension watches all of these files — CLI writes and hand edits alike
show up live, no reload needed. Bump \`updatedAt\`, keep \`status\`/\`progress\` honest while \`live\`, and
