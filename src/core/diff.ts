...
function splitListItems(group: string[]): MarkdownBlock[] {
  const baseIndent = (LIST_MARKER.exec(group[0]) as RegExpExecArray)[1].length;
  const items: string[][] = [];
...
export function diffMarkdown(before: string, after: string): DiffBlock[] {
  ...
  for (const [oi, ni] of common) {
    while (o < oi) {
      out.push({ op: 'del', block: oldBlocks[o++] });
    }
    while (n < ni) {
      out.push({ op: 'add', block: newBlocks[n++] });
    }
    out.push({ op: 'same', block: newBlocks[ni] });
    o = oi + 1;
    n = ni + 1;
  }
...
export function runSource(run: DiffRun): string {
  const parts = run.blocks.map((block) =>
    block.ordinal === undefined
      ? block.text
      : block.text.replace(ORDERED_MARKER, (_m, indent, _n, tail) => `${indent}${block.ordinal}${tail}`),
  );
