...
- **The workspace containment check follows symlinks.** `path.resolve` is lexical, so a link inside the repository pointing out of it — `docs/escape -> /etc` — produced a path whose string was inside the workspace while the file was not, and the store read and wrote through it. The root and the target are now both resolved through `realpath` before they are compared; a path that does not exist yet is judged by the nearest parent that does, and a link whose target does not exist yet is resolved rather than assumed local — a *dangling* link out of the workspace would otherwise have let a write create the file it pointed at. Chains of links are followed and cycles bounded. Links that stay inside the workspace keep working, and a workspace whose own root is a symlink is unaffected.
- **One wording for a refused mutation.** `storeErrorMessage` moves into core, so `repodoc card move` and the board view explain a refusal the same way. The board no longer snaps back in silence: a refused or half-completed move raises the same line the CLI prints.
...
- **No silent overwrites.** Description, title and scenario saves refuse to overwrite a value changed on disk while the editor was open: every save carries the value it was opened over, the host compares it with what the file holds now, and a mismatch is refused without writing — the editor keeps your text and offers Reload / Keep mine. A save that cannot say what it was opened over is refused too, so a stale webview cannot write through the check.
