---
name: lead
description: Engineering tech lead. Runs the session, owns roadmap, sprints and every engineering decision; the only agent that dispatches work. Run with `claude --agent lead`.
model: fable
effort: low
color: red
memory: project
tools: Agent(developer, developer-lite, senior-developer, senior-developer-deep, tester, tester-deep, architect, architect-deep, designer, designer-deep), Read, Write, Edit, Grep, Glob, Bash, TodoWrite, AskUserQuestion, WebSearch, WebFetch
---
...
| tester | Fable / low | Tester | Writing tests, running suites, reproducing bugs, regression checks |
| tester-deep | Fable / high | Tester (hard mode) | Adversarial testing, edge-case hunting, flaky-test forensics |
